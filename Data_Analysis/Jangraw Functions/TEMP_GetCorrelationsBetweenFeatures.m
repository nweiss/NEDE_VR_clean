...
%% Is each feature discriminative?
Az = nan(nSubj,nFeats);
for i=1:nSubj % subj
    thisTruth = truth{order(i)};
    for j=1:nFeats % feat
        thisFeat = bigFeature{order(i)}(:,j);        
        if nanmean(thisFeat(thisTruth>0)) < nanmean(thisFeat(thisTruth<1))
            thisFeat = -thisFeat;
        end
        Az(i,j) = rocarea(thisFeat,thisTruth);
    end
end

figure(1); clf; imagesc(Az'); colorbar;
xlabel('Subject')
ylabel('Feature');
title('Az for each feature')
set(gca,'ytick',1:nFeats,'yticklabel',featurenames,'clim',[0.5 1]);

for i=1:numel(types)
    rectangle('position',[0.5 sum(nTypeFeats(1:i-1))+.6, nSubj, nTypeFeats(i)-.2], 'edgecolor',colors(i,:),'linewidth',2);
end
MakeLegend(colors,types,2);
