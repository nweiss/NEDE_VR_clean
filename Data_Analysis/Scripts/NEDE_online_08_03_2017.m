...
%   2) 'matlab+unity' - run t                     he script with an established connection to
...
MODE = 'matlab';%+unity+inputstreams';%+inputstreams';
UNITY_PYTHON = true;
EEG_EYE = true;
...
if strcmp(MODE, 'matlab')
    ...
    load(LOAD_PATH);
    % Because billboard_id can be zero, make default nan when billboard
    % offscreen
    billboard_offscreen_ind = unity.time_series(5,:)==0;
...
nTargetsPassed = 0;
Billboard.ids_passed = unique(unity_data(7,:));
Billboard.ids_passed(Billboard.ids_passed == 0) = [];
